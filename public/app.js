import { fetchBurgerKingLocations } from "./lib/burgerking.js";
...
// Fetch and render BKs for current viewport
async function loadBurgerKingsForView() {
  if (map.getZoom() < 6) return; // avoid huge initial fetch

  const bounds = map.getBounds();
  const burgerKings = await fetchBurgerKingLocations({
    ...
    const marker = new maplibregl.Marker(el)
      ...
      .setPopup(
        new maplibregl.Popup({ offset: 25 }).setHTML(`
          ...
          Lat: ${bk.lat.toFixed(4)}<br/>
          Lon: ${bk.lon.toFixed(4)}
        `)
      )
      .addTo(map);
...
map.on("load", loadBurgerKingsForView);
map.on("moveend", loadBurgerKingsForView);
